...
testR can be installed from *CRAN* repository. 
...
## Capturing and generating tests for arbitrary code

Until now, we have always worked with entire package. But there may be cases in which you do not want to limit yourself to create tests or add regressions for a specified package. testR thus allows you to use all the underlying functions used in the process of the above examples so that you can capture any functions and any code. This section briefly explains the functions to accomplish this in order of their logical application:

```R
capture(..., verbose = testr_options("verbose"))
```

`capture` functions allows specified functions from different packages to be captured (i.e. tests for their usage will be generated). The function names are accepted in either characters, or in R's symbols, i.e. `package:::function`. 
