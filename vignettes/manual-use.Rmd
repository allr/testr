...
```R
start_capture(..., verbose = testr_options("verbose"))
```

`start_capture` function allows specified functions from different packages to be captured (i.e. tests for their usage will be generated). The function names are accepted in either characters, or in R's symbols, i.e. `package:::function`.
