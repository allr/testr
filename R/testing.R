...
reg_tests <-  function(exec_dir = tempdir(),
                       file_patterns, file_indexes,
                       verbose = testr_options("verbose"),
                       parallel_test = testr_options("parallel_tests")) {
    test_dir <- normalizePath(switch(basename(getwd()),
                                     "tests" = "reg-tests",
                                     "testr" = "inst/tests/reg-tests",
                                     "testthat" = "reg-tests"))
    invisible(runner(test_dir, file_patterns, file_indexes, exec_dir, verbose = verbose))
}
...
#' @param verbose if to print additional information (default \code{TRUE})
#'
runner <- function(test_dir, file_patterns, file_indexes,
