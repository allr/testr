...
public class TestGen {
  ...
  private static void processFiles() {
    ...
    try {
      if (!Files.isDirectory(inputDir)) {
        ...
        DirectoryStream<Path> seeds = Files.newDirectoryStream(inputDir);
        for (Path seed : seeds) {
  ...
  private static HashMap<String, HashSet<FuncEntry>> processFile(Path seed)
    ...
    assert (linesArr.length == 0 || linesArr[0].startsWith(FUNC_PREFIX));
    int[] s = { 0 };
  ...
  private static FuncEntry processEntry(String[] lines, int[] ss) {
    ...
    entry.func = lines[s++].substring(FUNC_PREFIX.length());
    assert (s < lines.length);
    entry.type = lines[s++].substring(TYPE_PREFIX.length());
    assert (s < lines.length);
    entry.args = "";
  ...
  static class FuncEntry {
    ...
    @Override
    public int hashCode() {
      return func.hashCode() ^ type.hashCode() ^ args.hashCode()
          ^ retn.hashCode();
    }
    ...
    public boolean equals(Object o) {
      ...
      return o1.func.equals(func) && o1.type.equals(type)
